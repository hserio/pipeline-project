...
#get logging set up
logging.basicConfig(filename='PipelineProject.log', level=logging.INFO, format='%(asctime)s - %(message)s')
...
#initialize list of fastq pairs
fastq_pairs = []
...
for file in infile:
    ...
    with open(file) as input_file:
        ...
        #match up 1 & 2 fastq files to enter them correctly in the list of lists
...
mapped_fastq = []
for file in sorted(os.listdir()): #sort list so files are in correct order for spades ex: (hannah_1.fastq, hannah_2.fastq)
...
with open(f"contigs.fasta") as fasta_file: #read contigs fasta file
    for record in SeqIO.parse(fasta_file, "fasta"):
        if len(record.seq) > 1000:
            ...
            assembly_length += len(record.seq) #sum basepairs for all contigs over 1000 bp
...
#create a database using the dataset from the subfamily
...
#get output file
hits = "myresults.csv"
#add each hit from the .csv to the log file
